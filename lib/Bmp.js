...
class Bmp {
  ...
  invert(imagePath) {
    ...
    const { body, header } = this.#processImage(imagePath);
    const arr = [];
  ...
  blur(imagePath, blurAmount) {
    const { body, header, bitDepth, width } = this.#processImage(imagePath);
    ...
    body.forEach((row, i) => {
      row.forEach((pixel, index) => {
        if (bitDepth === 3 && row.length > width && index === row.length - 1)
          return arr.push(row[row.length - 1]);
        ...
        for (let x = -amount; x <= amount; x++) {
          for (let y = -amount; y <= amount; y++) {
            ...
            if (
              bitDepth === 3 &&
              row.length > width &&
              body[i + x][index + y] === row[row.length - 1]
            )
              continue;
