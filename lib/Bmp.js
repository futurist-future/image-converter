...
class Bmp {
  ...
  #processImage(imagePath) {
    ...
    const headerLength =
      hexToDecimal(dataString.slice(28, 30)) === 40 ? 108 : 124;
    ...
    const width = hexToDecimal(littleEndian(header.slice(36, 44)));
    const height = hexToDecimal(littleEndian(header.slice(44, 52)));
    ...
    const bodyDivided = splitPerIndex(body, 2);
    const bodyRow = splitIntoPart(bodyDivided, width);
    const hexArr = bodyRow.map((row) => {
      return splitPerIndex(row, 3);
    });

    const firstRow = hexArr[0]
    const lastRow = hexArr[hexArr.length - 1];

    // const bodyRow = splitPerIndex(hexArr, width);

    //bodyRow.forEach((item) => {
    // while (((item.length * colorLength) / 2) % 4 !== 0) {
    //   item.push("00");
    // }
    // console.log(item.length * 3 / 4);
  ...
  toBnW(imagePath) {
    ...
    body.forEach((row) => {
      row.forEach((pixel) => {
        arr.push(hexAverage(pixel, colorLength));
      });
    });
