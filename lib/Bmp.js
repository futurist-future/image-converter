...
class Bmp {
  ...
  #processImage(imagePath) {
    ...
    // const rowSize = (bitDepth * (width + adder)) / 32;
    ...
    // const hexArr = splited.map((row) => {
    //   return splitPerIndex(row, 4);
    // });

    // console.log(bodyRow.length);
  ...
  toBnW(imagePath) {
    ...
    body.forEach((row) => {
      row.forEach((pixel) => {
        // console.log(hexAverage(pixel, bitDepth / 4));
        arr.push(hexAverage(pixel, bitDepth / 4));
      });
    });
